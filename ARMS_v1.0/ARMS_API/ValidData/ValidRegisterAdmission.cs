...
namespace ARMS_API.ValidData
{
    public class ValidRegisterAdmission
    {
        ...
        public ValidRegisterAdmission(UserInput userInput, IStudentProfileRepository studentProfileRepository)
        {
        ...
        public async void InputAddRegisterAdmission(RegisterAdmissionProfileDTO registerAdmissionProfileDTO)
        {
            ...
            if (await _studentProfileRepository.isExistEmailStudent(registerAdmissionProfileDTO.EmailStudent)) throw new Exception("Email đã được đăng ký!");
            // phone number
            ...
            if (await _studentProfileRepository.isExistEmailStudent(registerAdmissionProfileDTO.PhoneStudent)) throw new Exception("Số điện thoại đã được đăng ký!");
            // CID
            ...
            if (await _studentProfileRepository.isExistEmailStudent(registerAdmissionProfileDTO.CitizenIentificationNumber)) throw new Exception("Số căn cước công dân đã được đăng ký!");
            //CI address
            ...
            // major
            if (string.IsNullOrEmpty(registerAdmissionProfileDTO.Major1)) throw new Exception("Vui lòng chọn ngành học!");
            // SpecializeMajor
            if (string.IsNullOrEmpty(registerAdmissionProfileDTO.SpecializeMajor1)) throw new Exception("Vui lòng chọn chuyên ngành!");
            // YearOfGraduation
